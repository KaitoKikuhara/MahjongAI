...
goal_average_reward = 0.9  # この報酬を超えると学習終了
...
for episode in range(num_episodes):  # 試行数分繰り返す
    ...
    if(syanten == 0):
        total_reward_vec = np.hstack((total_reward_vec[1:], 1))
    else:
        total_reward_vec = np.hstack((total_reward_vec[1:], 0))
